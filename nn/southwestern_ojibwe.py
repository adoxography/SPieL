...
DATA_TGZ = os.path.join(DATA_PATH, 'swo-corpus.tgz')
...
def _train_data_filenames(tmp_dir, with_segmented=True):
    ...
    """
    listing = [
        (os.path.join(tmp_dir, 'swo-corpus', 'train', 'swo-train.original'),
         os.path.join(tmp_dir, 'swo-corpus', 'train', 'swo-train.segmented'))
    ]
...
def _dev_data_filenames(tmp_dir, with_segmented=True):
    ...
    """
    listing = [
        (os.path.join(tmp_dir, 'swo-corpus', 'dev', 'swo-dev.original'),
         os.path.join(tmp_dir, 'swo-corpus', 'dev', 'swo-dev.segmented'))
    ]
...
class LanguageModelSouthwesternOjibwe(text_problems.Text2SelfProblem,
...
@registry.register_problem
class SegmentSouthwesternOjibwe(segmentation.SegmentationProblem):
    """
...
@registry.register_problem
class RecognizeSouthwesternOjibwe(recognition.RecognitionProblem):
    """
...
@registry.register_problem
class MultitaskSouthwesternOjibwe(multi_problem.MultiProblem,
                                  util.SingleProcessProblem):
    """
    ...
    def __init__(self, was_reversed=False, was_copy=False):
        super(MultitaskSouthwesternOjibwe, self).__init__(was_reversed,
                                                          was_copy)
        self.task_list.append(LanguageModelSouthwesternOjibwe())
        self.task_list.append(SegmentSouthwesternOjibwe())
...
@registry.register_problem
class MultitaskSouthwesternOjibweWithNoise(MultitaskSouthwesternOjibwe):
    """
    Defines a multitask problem that trains a language model, a segmentation
    task, and a recognition task
    """
    def __init__(self, was_reversed=False, was_copy=False):
        super(MultitaskSouthwesternOjibweWithNoise,
              self).__init__(was_reversed,
                             was_copy)
        self.task_list.append(RecognizeSouthwesternOjibwe())
